...
import { BankQuestion, StemFormat, Difficulty, BankChoice } from '../../types/questionBank.types';
...
interface Props {
  ...
  onClose: () => void;
  onSubmit: (updated: Partial<BankQuestion>) => Promise<void>;
  isSubmitting: boolean;
  onDelete?: () => Promise<void>;
  isDeleting?: boolean;
}

export default function BankQuestionManualEditModal({ question, onClose, onSubmit, isSubmitting, onDelete, isDeleting }: Props) {
  const [stem, setStem] = useState(question.stem);
  const [stemFormat, setStemFormat] = useState<StemFormat>(question.stemFormat);
  ...
  // MCQ specific
  const [choices, setChoices] = useState<BankChoice[]>(question.type === 'mcq' ? [...question.choices] : [
    { label: 'A', text: '' },
    { label: 'B', text: '' },
    { label: 'C', text: '' },
    { label: 'D', text: '' },
  ...
  const removeChoice = (indexToRemove: number) => {
    ...
    const removedChoiceText = choices[indexToRemove].text;
    if (correctAnswer === removedChoiceText) {
      setCorrectAnswer(newChoices[0].text);
    }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const path = `bank_images/${uuidv4()}.${ext}`;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const updated: Partial<BankQuestion> = {
      stem,
      stemFormat,
      explanation,
      difficulty,
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
             <div>
               ...
               <select
                 value={stemFormat}
                 onChange={e => setStemFormat(e.target.value as StemFormat)}
                 ...
                 <div key={c.label} className="flex gap-2 relative group">
                    <div className="flex-shrink-0 w-10 flex flex-col justify-center items-center">
                       ...
                       <input 
                         type="radio" 
                         name="correctAnswer" 
                         checked={correctAnswer === c.text}
                         onChange={() => setCorrectAnswer(c.text)}
