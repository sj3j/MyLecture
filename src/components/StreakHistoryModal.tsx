...
export default function StreakHistoryModal({ student, isOpen, onClose, lang }: StreakHistoryModalProps) {
  ...
  // Simple Github style calendar: 7 days a week, 12 weeks
  // We'll generate the last 84 days (12 weeks * 7 days)
  ...
  for (let i = 83; i >= 0; i--) {
    ...
    const dateStr = `${ey}-${em.toString().padStart(2, '0')}-${ed.toString().padStart(2, '0')}`;
          ...
          {isLoading ? (
            ...
            <div className="space-y-8">
              <div className="flex justify-between items-center">
                <h3 className="font-bold text-slate-800 dark:text-slate-200">{isRtl ? 'آخر 12 أسبوع' : 'Last 12 weeks'}</h3>
                <button
              ...
              <div className="grid grid-cols-[repeat(12,minmax(0,1fr))] gap-2 sm:gap-4 overflow-x-auto pb-4">
                {Array.from({ length: 12 }).map((_, weekIndex) => (
                  <div key={weekIndex} className="grid grid-rows-7 gap-1 sm:gap-2">
                    {Array.from({ length: 7 }).map((_, dayIndex) => {
                      const dayStr = calendarDays[weekIndex * 7 + dayIndex];
                      if (!dayStr) return null;
                      const record = history.find(h => h.date === dayStr);
                      // Gray = inactive, orange = active, blue/cyan = freeze
                      let bgClass = "bg-slate-100 dark:bg-zinc-800"; // default inactive
                      if (record?.wasActive && record?.freezeUsed) bgClass = "bg-sky-400 dark:bg-sky-500";
                      else if (record?.wasActive) bgClass = "bg-orange-500 dark:bg-orange-600";
                      
                      return (
                        <div 
                          key={dayStr}
                          title={`${dayStr}: ${record?.wasActive ? (record.freezeUsed ? 'Freeze Used' : 'Active') : 'Inactive'}`}
                          className={`w-4 h-4 sm:w-6 sm:h-6 rounded-sm sm:rounded-md ${bgClass} cursor-pointer hover:ring-2 hover:ring-slate-400 transition-all`}
                        />
                      );
                    })}
                  </div>
              ...
              <div className="flex gap-4 items-center justify-center text-xs text-slate-500">
                <div className="flex items-center gap-1"><div className="w-3 h-3 bg-slate-100 dark:bg-zinc-800 rounded-sm"></div> Inactive</div>
                <div className="flex items-center gap-1"><div className="w-3 h-3 bg-orange-500 dark:bg-orange-600 rounded-sm"></div> Active</div>
                <div className="flex items-center gap-1"><div className="w-3 h-3 bg-sky-400 dark:bg-sky-500 rounded-sm"></div> Freeze Used</div>
              </div>
